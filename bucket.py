...
def event(event, context):
    ...
    for record in event["Records"]:
        ...
        for rule in open("/tmp/{}".format(key)):
            ...
            if (
                len(tokens) > 2
                and tokens[0] in ["add", "remove"]
                and tokens[1] == "assetgroup"
            ):
                ...
                len(tokens) >= 5
                and tokens[0] == "assetgroup"
                and tokens[1] == "matches"
                and tokens[3] == "link"
                and tokens[4] == "service"
            ):
        ...
        for rule in rules:
            # debug
            print(rule.ruletype, rule.action, rule.tokens)
            if rule.ruletype == "addService":
                ...
                service = None
                service_matches = [
                    s for s in Service.scan(name__eq=" ".join(rule.tokens[2::]))
                ]
            ...
            if rule.ruletype == "assetLinkAssetgroup":
                ...
                for group in groups:
                    ...
                    assets = Asset.scan(
                        asset_identifier__contains=asset_identifier
                    ).recursive()
                    for asset in assets:
                        if group.assets:
